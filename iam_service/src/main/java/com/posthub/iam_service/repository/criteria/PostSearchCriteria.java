...
public class PostSearchCriteria implements Specification<Post> {
    ...
    public Predicate toPredicate(
        ...
        if (Objects.nonNull(request.getTitle()))
            predicateList.add(criteriaBuilder.like(root.get(Post.TITLE_NAME_FIELD), "%" + request.getTitle() + "%"));

        if (Objects.nonNull(request.getContent()))
            predicateList.add(criteriaBuilder.like(root.get(Post.CONTENT_NAME_FIELD), "%" + request.getContent() + "%"));

        if (Objects.nonNull(request.getLikes()))
            predicateList.add(criteriaBuilder.equal(root.get(Post.LIKES_NAME_FIELD), request.getLikes()));

        if (Objects.nonNull(request.getDeleted()))
            predicateList.add(criteriaBuilder.equal(root.get(Post.DELETED_NAME_FIELD), request.getDeleted()));
