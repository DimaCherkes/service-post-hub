...
public class UserControllerTest {
    ...
    void create_User_AsAdmin_OK_200() throws Exception {
        ...
        IamResponse<UserDTO> response = parseUserDTOResponse(responseResult.getResponse().getContentAsByteArray());

        UserDTO resultBody = Objects.nonNull(response.payload()) ? response.payload() : null;
    ...
    void updateUser_AsAdmin_OK_200() throws Exception {
        UpdateUserRequest request = new UpdateUserRequest("newUserName", "updatedUser@gmail.com");
        mockMvc.perform(MockMvcRequestBuilders
                        .put("/users/1")
                        .header(HttpHeaders.AUTHORIZATION, adminJwt)
                        .contentType(MediaType.APPLICATION_JSON)
