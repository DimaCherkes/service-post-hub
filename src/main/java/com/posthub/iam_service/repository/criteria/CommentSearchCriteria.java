...
public class CommentSearchCriteria implements Specification<Comment> {
    ...
    public Predicate toPredicate(
        ...
        if (Objects.nonNull(request.getMessage()))
            predicateList.add(criteriaBuilder.like(root.get(Comment.MESSAGE_NAME_FIELD), "%" + request.getMessage() + "%"));

        if (Objects.nonNull(request.getCreatedBy()))
            predicateList.add(criteriaBuilder.like(root.get(Comment.CREATED_BY_FIELD), "%" + request.getCreatedBy() + "%"));

        if (Objects.nonNull(request.getDeleted()))
            predicateList.add(criteriaBuilder.equal(root.get(Comment.DELETED_FIELD), request.getDeleted()));

        if (Objects.nonNull(request.getPostId()))
            predicateList.add(criteriaBuilder.equal(root.get("post").get("id"), request.getPostId()));
